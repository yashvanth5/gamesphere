...
export const Cart =()=>{
const {cartState ,removeFromCart,updateQuantityHandler,totalPrice,discountedPrice} = useContext(CartContext)
...
{cartState?.cart.length >0 ?     <div className="cart-block" >  
    <div className="cart-items">
        {cartState.cart?.map((game)=>{
               const {title,price,image,_id,starRatings,discountPercent,qty,discountPrice} = game
...
<p class="discount-price ">{ price===0 ? 'Free' : `₹ ${price-discountPrice}`}</p>
              ...
              {price===0? "" : <div className="cart-discount">
...
<button className="cart-quantity-decrease"   disabled={qty ==1 } onClick={()=>updateQuantityHandler( game,"decrement")}>-</button>
<p className="cart-qunatity-live-count"> {qty > 0 ? qty : 1}</p>
<button className="cart-quantity-increase"  onClick={()=>updateQuantityHandler( game,"increment")} >+</button>
</div>

<button className="remove-cart-btn" onClick={()=>removeFromCart(game)}>Remove From Cart</button>
...
{isAlreadyInWishlist ?    <Link onClick={()=>removeFromWishlist(game)}   >
    ...
    <div  className="cart-block-price-detail" >
        ...
        {cartState.cart.length >1 ? <h2 className="cart-block-price-details-count">({cartState.cart.length} items)</h2> :
...
<hr/>
    ...
    <button className="card-block-price-details-final-btn" > <h2>Checkout</h2></button>
  </div>
